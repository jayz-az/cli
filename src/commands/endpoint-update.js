...
module.exports = {
  command: 'endpoint update <learnUrl>',
  desc: 'Re-generate an endpoint from a Microsoft Learn REST doc URL (overwrites existing).',
  builder: (y) => y
    .positional('learnUrl', { type: 'string', describe: 'The Learn REST doc URL to scrape.' })
    .option('force', { type: 'boolean', default: true, describe: 'Overwrite existing (always true for update).' }),
  handler: async (argv) => {
    try {
      ...
      console.error('endpoint update failed:', err.message);
      process.exit(1);
    }
  },
};
