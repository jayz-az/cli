...
const { readConfig, writeConfig, CONFIG_FILE } = require('../config');
...
function ask(rl, q, defVal) {
  ...
  return new Promise((resolve) => rl.question(prompt, (a) => resolve(a || defVal || '')));
}
...
async function interactiveInit(argv) {
  ...
  try {
    const clientId = await ask(rl, 'Azure AD App Client ID', process.env.JAYZ_CLIENT_ID || existing.clientId || '');
    const tenantId = await ask(rl, 'Tenant ID', process.env.JAYZ_TENANT_ID || existing.tenantId || '');
    const subscriptionId = await ask(rl, 'Default Subscription ID (optional)', process.env.JAYZ_SUBSCRIPTION_ID || existing.subscriptionId || '');
    const authorityHost = await ask(rl, 'Authority Host (optional, e.g., https://login.microsoftonline.us)', process.env.JAYZ_AUTHORITY_HOST || existing.authorityHost || '');
