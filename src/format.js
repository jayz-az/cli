...
function extractLocation(item) {
  if (!item || typeof item !== 'object') return undefined;
  return item.location || (item.properties && item.properties.location);
}

function extractType(item) {
  if (!item || typeof item !== 'object') return undefined;
  return item.type || (item.properties && item.properties.type);
}
...
function computeColumns(rows) {
  const preferred = ['name', 'resourceGroup', 'location', 'type', 'id'];
...
function pad(str, len) {
  ...
  return s + (needed > 0 ? ' '.repeat(needed) : '');
}
